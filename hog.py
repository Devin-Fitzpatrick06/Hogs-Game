...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    #(opponent_score//10)%10 != True
    if (3 * abs(player_score%10 - (opponent_score//10)%10)) < 1:
        return 1
    #elif player_score < 10 or opponent_score < 10:
...
def sus_points(score):
    ...
    #if score == 1:
        #return 1
    #elif is_prime(score) == True:
        #return score
    if num_factors(score) == 3 or num_factors(score) == 4:
        i = 1
        while i > 0:
            if num_factors(score+i) == 2:
                return score+i
            else:
                i+=1
    else:
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while True:
        if who == 0:
            player0_rolls = strategy0(score0, score1)
            print("DEBUG:", player0_rolls)
            score0 = update(player0_rolls, score0, score1, dice)
            print("DEBUG:",score0)
            who += 1
            if score0 >= goal:
                return score0, score1
        else:
            player1_rolls = strategy1(score1, score0)
            print("DEBUG:", player1_rolls)
            score1 = update(player1_rolls, score1, score0, dice)
            print("DEBUG:",score1)
            who -= 1
            if score1 >= goal:
                return score0, score1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            print("DEBUG:", strategy(score, opponent_score))
            if strategy(score,opponent_score) != first:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, times_called=1000):
    ...
    def f(*args):
        ...
        for i in range(times_called):
            totals += original_function(*args)
        print("DEBUG:",*args)
        print("DEBUG:", original_function)
        return totals/times_called
    return f
    # END PROBLEM 8
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if sus_update(0, score, opponent_score) >= score + threshold:
        ...
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
#sus_update(num_rolls, score, opponent_score)
